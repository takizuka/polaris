...
import {getPropsThatCanHaveChildren, propPathsAreEqual} from '../../utils';
    ...
    >
      ...
      {propsWithChildren.length > 0 && (
        <ul className={styles.Children}>
          {propsWithChildren.map(({propPath}) => {
            ...
            const tooltipId = `add-layer-${layer.id}-${propPath}`;
            return (
              <li key={propPath.toString()}>
                {propPath.map(({node}) => node).join('.')}{' '}
                <button
